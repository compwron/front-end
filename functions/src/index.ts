...
export const t = functions.https.onRequest((req, res) => {
	
	// https.get({ hostname: "http://www.google.com" }, r => {
	// 	r.pipe(res)
	...
	pipe.on('data', c => response.push(c))
	return pipe.on('end', () => { return res.send(Buffer.concat(response)) })
	// res.send(null)
		// .then(response => {
